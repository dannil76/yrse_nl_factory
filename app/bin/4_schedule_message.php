...
---------------------------------------------------------------------------------
 Epsilon Harmony API - Schedule message by Dan Nilsson 2018 (mail@dannilsson.se)
---------------------------------------------------------------------------------
<?php
...
array_map( 'unlink', glob( QUEUE_PATH . '*' ) );
	...
	foreach( $recipeData as $site => $params )
	{
		...
		if( is_array( $params ) )
		{
			...
			foreach( $params as $param )
			{
				...
				$seedListId		= getSetting('seedlist_' . $langCode);
				...
				$messageSendDate		= $param['send_date'];
				...
				$jsonBody = [
					'id'				=> $messageId,
					'sendDate'			=> $messageSendDate,
					'seedListId'		=> $seedListId,
					'lang'				=> $langCode,
				];

				if( array_key_exists('override_time', $param) )
					$jsonBody['override_time'] = true;

				$outputFile = $siteIndex . '_' . strtoupper($langCode) . '_SCHEDULE_' . $messageName . '.json';
