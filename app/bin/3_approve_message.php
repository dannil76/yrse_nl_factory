...
--------------------------------------------------------------------------------
 Epsilon Harmony API - Approve message by Dan Nilsson 2018 (mail@dannilsson.se)
--------------------------------------------------------------------------------
<?php
...
array_map( 'unlink', glob( QUEUE_PATH . '*_APPROVE_*' ) );
	...
	foreach( $recipeData as $site => $params )
	{
		...
		if( is_array( $params ) )
		{
			...
			foreach( $params as $param )
			{
				...
				$outputFile = $siteIndex . '_' . strtoupper($langCode) . '_APPROVE_' . $messageName . '.json';
